...
if(isset($_POST[tr_submit]))
{
	...
	{
			$sql ="UPDATE treatment_records SET appointmentid='$_POST[select2]',treatmentid='$_POST[select4]',patientid='$_POST[select3]',doctorid='$_POST[select5]',treatment_description='$_POST[textarea]',uploads='$filename',treatment_date='$_POST[treatmentdate]',treatment_time='$_POST[treatmenttime]',status='Active' WHERE appointmentid='$_GET[editid]'";
	...
	{
		$sql ="INSERT INTO treatment_records(appointmentid,treatmentid,patientid,doctorid,treatment_description,uploads,treatment_date,treatment_time,status) values('$_POST[select2]','$_POST[select4]','$_POST[select3]','$_POST[select5]','$_POST[textarea]','$filename','$_POST[treatmentdate]','$_POST[treatmenttime]','Active')";
      ...
      <tbody>
        <tr>
          ...
          <select name="select2" id="select2">
           <option value="">Select</option>
            <?php
		  	$sqlappointment= "SELECT * FROM appointment WHERE status='Active'";
			$qsqlappointment = mysqli_query($con,$sqlappointment);
			while($rsappointment=mysqli_fetch_array($qsqlappointment))
			{
				if($rsappointment[appointmentid] == $rsedit[appointmentid])
				{
				echo "<option value='$rsappointment[appointmentid]' selected>$rsappointment[appointmentid]  - ( $rsappointment[patientname])</option>";
				}
				else
				{
					echo "<option value='$rsappointment[appointmentid]'>$rsappointment[appointmentid]  - ( $rsappointment[patientname])</option>";
				}
        ...
        <tr>
          ...
          <select name="select3" id="select3">
           <option value="">Select</option>
            <?php
		  	$sqlappointment= "SELECT * FROM patient_detail ";
			$qsqlappointment = mysqli_query($con,$sqlappointment);
			while($rsappointment=mysqli_fetch_array($qsqlappointment))
			{
				if($rsappointment[patientid] == $rsedit[patientid])
				{
				echo "<option value='$rsappointment[patientid]' selected>$rsappointment[patientid]  - ( $rsappointment[patientname])</option>";
				}
				else
				{
					echo "<option value='$rsappointment[patientid]'>$rsappointment[patientid]  - ( $rsappointment[patientname])</option>";
				}
        ...
        <tr>
          <td colspan="2" align="center"><input type="submit" name="tr_submit" id="tr_submit" value="Submit" /> </td>
        </tr>
      </tbody>
    </table>
    </form>
